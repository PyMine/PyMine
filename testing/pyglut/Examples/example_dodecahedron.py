...
from pyglut import Dodecahedron 

from pyglut import Vertex 
...
def mainloop() :
  ...
  test_dodecahedron=Dodecahedron(15.5,display_mode="lined",lines_color=Color(ub_v=(127,127,127,0)),faces_color=color_faces,lines_width=line_width,display_ls=True)
  i=5.625/4.0
  m=Matrix()
  m.translate((15.5,0.0,0.0))
  test_dodecahedron.update_pos(m)
  while True :
    
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)  
    
    m=Matrix()
    m.translate((-test_dodecahedron.center.wx,-test_dodecahedron.center.wy,-test_dodecahedron.center.wz))
    m.rotate_x(i)
    m.rotate_y(i)
    m.rotate_z(i)
    m.translate((test_dodecahedron.center.wx,test_dodecahedron.center.wy,test_dodecahedron.center.wz))
    ...
    for event in pygame.event.get() :
	  ...
	  display_mode += 1
	  if display_mode == 1 :
	    test_dodecahedron.set_display_mode("faced")
	  elif display_mode == 2 :
	    test_dodecahedron.set_display_mode("twice")  
	  elif display_mode == 3 :
	    display_mode=0
	    test_dodecahedron.set_display_mode("lined")
	    
	  
	elif event.key == K_l :
	  test_dodecahedron.set_lines_color(Color(ub_v=(randint(63,255),randint(63,255),randint(63,255))))
	  
	elif event.key == K_f :
	  if faces_color_mode == 1 :
	    test_dodecahedron.set_faces_color(Color(ub_v=(randint(63,255),randint(63,255),randint(63,255))))
	    faces_color_mode=0
	  elif faces_color_mode == 0 :
	    color_faces=[]
            for v in range(0,12) :
              color_faces.append(Color(ub_v=(randint(63,255),randint(63,255),randint(63,255))))
	    test_dodecahedron.set_faces_color(color_faces) 
	    faces_color_mode=1
	    
	elif event.key == K_UP :
	  line_width += 1
	  
	  test_dodecahedron.set_lines_width(line_width)
	  ...
	  test_dodecahedron.set_display_ls(bool_display_ls)
	
	elif event.key == K_s :
	  test_dodecahedron.set_side_length(uniform(1.0,20.0))
	  m=Matrix()
          m.translate((15.5,0.0,0.0))
          test_dodecahedron.update_pos(m)
